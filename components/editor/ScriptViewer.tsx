...
export default function ScriptViewer({ script, currentTime, onSeek, isOpen, onClose, onUpdateScript }: ScriptViewerProps) {
    ...
    const startEditing = (index: number, line: ScriptLine) => {
        ...
        setEditStart(line.startTime);
        setEditEnd(line.endTime);
        onSeek(line.startTime); // Also seek to the start time
    };
    ...
    const saveEdit = (index: number, e: React.MouseEvent) => {
        ...
        newScript.sort((a, b) => a.startTime - b.startTime);
    ...
    const addNewLine = () => {
        ...
        setEditStart(newLine.startTime);
        setEditEnd(newLine.endTime);
                    ...
                    <div className="relative export-menu">
                        <button
                            ...
                            className={cn(
                                ...
                                showExportMenu ? "bg-ember text-white" : "text-zinc-400 hover:text-white hover:bg-white/10"
                            )}
                    ...
                    return (
                        <div
                            ...
                            className={cn(
                                ...
                                isActive
                                    ? "bg-ember/10 border-ember/50 shadow-[0_0_15px_rgba(255,106,42,0.2)]"
                                    : "bg-zinc-900/50 border-white/5 hover:bg-white/5 hover:border-white/10",
                                isEditing ? "ring-2 ring-ember border-transparent" : "cursor-pointer group"
                            )}
                            style={{ padding: '8px' }}
                        >
                            <div className="flex justify-between items-start mb-1">
                                <span className={cn(
                                    ...
                                    isActive ? "bg-ember/20 text-ember-glow" : "bg-zinc-800 text-zinc-500"
                                )}>
                                    {Math.floor(line.startTime / 60)}:{Math.floor(line.startTime % 60).toString().padStart(2, '0')}
                                    {' - '}
                                    {Math.floor(line.endTime / 60)}:{Math.floor(line.endTime % 60).toString().padStart(2, '0')}
                                </span>
                            ...
                            {isEditing ? (
                                <div className="mt-2 space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full bg-black/50 text-white p-2 rounded border border-white/10 focus:outline-none focus:border-ember min-h-[60px] text-sm"
                                        autoFocus
                                        onClick={(e) => e.stopPropagation()}
                                    />
                                    <div className="flex justify-end items-center mt-2 gap-2">
                                        ...
                                        <button
                                            ...
                                            className="px-2 py-1 rounded bg-ember text-white hover:bg-orange-600 flex items-center gap-1 text-xs"
                                        >
                                            <Save size={12} /> Save
                                        </button>
                                    </div>
                                ...
                                <p className={cn("leading-relaxed text-sm", isActive ? "text-white" : "text-zinc-300")}>
                                    {line.text}
                                </p>
                            )}
                        </div>
